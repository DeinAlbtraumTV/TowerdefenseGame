...
public class UpgradeMenuButtonListener extends ListenerAdapter {
    ...
    public void onClickEvent(ClickEvent event) {
        ...
        TowerDefenseGame.componentContainer.getAddedComponents().forEach(comp -> {
            if (comp instanceof ButtonComponent) {
                ...
                if (buttonComp.getContent().equals("Towers")) {
        ...
        ButtonGroup.getButtonGroup("towerButtons").getButtons().forEach(button -> button.setVisibility(false));
        ButtonGroup.getButtonGroup("upgradeButtons").getButtons().forEach(button -> button.setVisibility(true));
    }
}
